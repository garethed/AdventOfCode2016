...
totally-real-room-200[decoy]";*/
            ...
            foreach (string line in input.Replace("\r", "").Split('\n'))
            {
                ...
                int startOfChecksum = line.IndexOf("[");

                string name = line.Substring(0, endOfName);
                string checksum = line.Substring(startOfChecksum + 1).TrimEnd(']');
                int sector = int.Parse(line.Substring(endOfName + 1, startOfChecksum - endOfName - 1));
        ...
        public static string Part2()
        {
            ...
            //string input = "qzmt-zixmtkozy-ivhz-343[aa]";
            ...
            foreach (string line in input.Replace("\r", "").Split('\n'))
            {
                ...
                string checksum = line.Substring(startOfChecksum + 1).TrimEnd(']');
                int sector = int.Parse(line.Substring(endOfName + 1, startOfChecksum - endOfName - 1));
                string decodedName = Decode(name, sector);

                if (ValidChecksum(name, checksum) && decodedName.Contains("pole"))
                {
                    ret += sector + ": " + decodedName + "\n";
                }
            }
        ...
        static string Decode(string encrypted, int sector)
        {
            string decoded = "";
        ...
        public static bool ValidChecksum(string name, string checksum)
        {
            name = name.Replace("-", "");
            var calculated = String.Join("",
                name.
                GroupBy(c => c).
