...
/dev/grid/node-x34-y24   92T   71T    21T   77%";
        ...
        {
            var lines = Utils.splitLines((string)input);
            var nodes = lines.Skip(2).Select(l => Node.fromString(l)).ToList();
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                ...
                {
                    if (nodes[j].used > 0 && nodes[i].free > nodes[j].used && i != j)
                    {
                        count++;
                    }
                }
        ...
        public string Part2(dynamic input)
        {
            ...
            var emptyNode = nodes.Where(n => n.used == 0).First();
            var blockedNodes = nodes.Where(n => n.used > 0 && n.used > emptyNode.free).ToDictionary(n => new Point(n.x, n.y), n => new List<Point>());

            var dataLocation = new Point(34, 0);

            var dataroute = shortestRoute(dataLocation, new Point(0, 0), blockedNodes);
            var emptyLocation = new Point(emptyNode.x, emptyNode.y);

            int steps = 0;

            foreach (var point in dataroute)
            {
                blockedNodes[dataLocation] = new List<Point>();
                steps += shortestRoute(emptyLocation, point, blockedNodes).Count;
                blockedNodes.Remove(dataLocation);
                emptyLocation = dataLocation;
                dataLocation = point;
                steps++;
            }
        ...
        private List<Point> shortestRoute(Point start, Point finish, Dictionary<Point, List<Point>> blockedNodes)
        {
            var used = new Dictionary<Point, List<Point>>(blockedNodes);
            var queue = new Queue<Point>();

            queue.Enqueue(start);
            used[start] = new List<Point>();
        ...
        private bool extendFrom(Point next, Point target, Dictionary<Point, List<Point>> used, Queue<Point> queue)
        {
            foreach (var adjacent in next.neighbours())
            {
                if (!used.ContainsKey(adjacent))
                {
                    var route = new List<Point>(used[next]);
                    route.Add(adjacent);
                    used[adjacent] = route;
...
/dev/grid/node-x0-y0     94T   65T    29T   69%
/dev/grid/node-x0-y1     88T   69T    19T   78%
/dev/grid/node-x0-y2     92T   12T    80T   78%
/dev/grid/node-x0-y3     93T   73T    20T   78%
/dev/grid/node-x0-y4     85T   73T    12T   85%
/dev/grid/node-x0-y5     94T   70T    24T   74%";

            Utils.Test(Part1, testinput, "10");
        }
        ...
        class Node
        {
            ...
            public static Node fromString(string line)
            {
                // Filesystem              Size  Used  Avail  Use%
                // /dev/grid/node-x0-y10    90T   72T    18T   80%
                var parts = Regex.Match(line, @".*\-x(\d+)\-y(\d+)\W+\d+T\W+(\d+)T\W+(\d+)T").Groups;
                return new Node()
                {
                    x = int.Parse(parts[1].Value),
                    y = int.Parse(parts[2].Value),
                    used = int.Parse(parts[3].Value),
                    free = int.Parse(parts[4].Value)
                };
            }
        }
        ...
        private class Point
        {
            ...
            {
                return x.GetHashCode() + 117 * y.GetHashCode();
            }
